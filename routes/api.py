...
def test_api():
    try:
        ...
        response = requests.request(
            ...
            verify=False  # Add this if dealing with self-signed certificates
        )
...
def export_api():
    ...
    if export_format == 'curl':
        curl_commands = []
        for api in apis:
            command = f"curl -X {api.method} '{api.url}'"
            ...
            if api.method in ['POST', 'PUT', 'PATCH'] and api.body:
                command += f" -d '{json.dumps(api.body)}'"
        ...
        response = jsonify(curl_commands)
        filename = f"{collection.name.lower().replace(' ', '_')}_curl.json"
